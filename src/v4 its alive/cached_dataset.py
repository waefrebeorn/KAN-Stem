...
def pad_or_truncate(waveform, target_length, n_fft):
...
class StemSeparationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Resample and convert to MelSpectrogram
        ...
        input_waveform = pad_or_truncate(input_waveform, self.target_length, self.n_fft)
        
        target_spectrograms = []

        for stem in range(self.num_stems):
            if idx >= len(self.stem_files[stem]):
                stem_spectrogram = torch.zeros_like(input_waveform)
            else:
                ...
                stem_waveform = pad_or_truncate(stem_waveform, self.target_length, self.n_fft)
                stem_spectrogram = stem_waveform

            target_spectrograms.append(stem_spectrogram)

        input_spectrogram = input_waveform
        target_spectrograms = torch.stack(target_spectrograms)
