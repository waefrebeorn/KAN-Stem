...
class KAN(nn.Module):
    def __init__(self):
        ...
        self.layer1 = KANLinear(128 * 256, 512)
        self.layer2 = KANLinear(512, 256)
        self.output_layer = nn.Linear(256, 128 * 256)
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.size(0), 128, 256)  # Reshape to spectrogram dimensions
        return x

def train(model, train_loader, criterion, optimizer, device, epoch, num_epochs, writer, checkpoint_dir, save_interval):
    model.train()
    running_loss = 0.0
...
def main():
    ...
    for epoch in range(args.num_epochs):
        train(model, train_loader, criterion, optimizer, device, epoch, args.num_epochs, writer, args.checkpoint_dir, args.save_interval)
