...
def separate_audio(input_audio, model_checkpoint, checkpoint_path, max_duration):
    ...
    if input_data is None:
        return "Error in preprocessing audio data."
    input_data = input_data.to(device)
    with torch.no_grad():
        separated_stems = model(input_data)
    output_stems = postprocess(separated_stems)

    # Converting the stems to smaller chunks if they are too large
    chunk_size = 44100  # Adjust chunk size if needed
    chunks = []
    for stem in output_stems:
        for i in range(0, len(stem), chunk_size):
            chunks.append(stem[i:i + chunk_size])
    print(f"Output chunks: {len(chunks)} with chunk size {chunk_size}")

    # Ensure that the output list has exactly 4 elements for Gradio
    if len(chunks) < 4:
        chunks += [np.zeros(chunk_size) for _ in range(4 - len(chunks))]
    elif len(chunks) > 4:
        chunks = chunks[:4]

    # Convert chunks to the format expected by Gradio
    chunk_paths = []
    for i, chunk in enumerate(chunks):
        path = f"chunk_{i}.wav"
        librosa.output.write_wav(path, chunk, sr=44100)
...
with gr.Blocks() as app:
    ...
    with gr.Tab("Separate Audio"):
        ...
        output_stems = [gr.Audio(type='file') for _ in range(4)]
